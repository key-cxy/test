  ...
  <div>
    ...
    >
      <el-button
        ...
        ><i class="el-icon-download el-icon--right"></i>导出</el-button
      >
      <el-table v-loading="loading" :data="list">
        ...
        >
          <template slot-scope="scope">
            <dict-tag
              :type="DICT_TYPE.BPM_TASK_ASSIGN_RULE_TYPE"
    ...
    >
      ...
      >
        ...
        <el-form-item label="分派方式" prop="assignType">
          <el-select v-model="form.assignType" clearable style="width: 100%">
            <el-option
              v-for="dict in assignTypeList"
              :key="parseInt(dict.value)"
              :label="dict.label"
              :value="parseInt(dict.value)"
            />
...
<script>
import { handleDownLoadTask } from "@/api/bpm/model";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        postIds: [
          { required: true, message: "指定岗位不能为空", trigger: "change" },
        ],
        userIds: [
        ...
        assignType: [
          { required: true, message: "分派方式不能为空", trigger: "change" },
        ],
      },
  ...
  methods: {
    ...
    async handleDownLoadTask() {
      ...
      const data = await handleDownLoadTask(param);
      // downloadFile (data, name)
      this.$download.excel(data, "分配规则.xls");
    },
    initModel(modelId) {
    ...
    init0() {
      ...
      // 获取分派方式
      this.assignTypeList = [];
      getAssignType().then((response) => {
        this.assignTypeList.push(...response.data);
      });
    },
    /** 获得任务分配规则列表 */
    getList() {
