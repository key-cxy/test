  ...
  methods: {
    resetListenersList() {
      ...
      this.bpmnElementListeners =
        this.bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type === `${this.prefix}:ExecutionListener` || ex.$type === 'activiti:executionListener') ?? [];
      this.elementListenersList = this.bpmnElementListeners.map(listener => initListenerType(listener));
      console.log('jiantingqi',  this.bpmnElement.businessObject)
    ...
    removeListenerField(field, index) {
      const that = this;
      this.$modal.confirm('确认移除该字段吗？').then(function() {
    ...
    removeListener(listener, index) {
      const that = this;
      this.$modal.confirm('确认移除该监听器吗？').then(function() {
        that.bpmnElementListeners.splice(index, 1);
        that.elementListenersList.splice(index, 1);
