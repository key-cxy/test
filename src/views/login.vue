...
import {encrypt1} from '@/libs/encrypt.js'
    ...
    handleLogin() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.loginForm.password = encrypt1(this.loginForm.password)
          Cookies.set("username", this.loginForm.username, { expires: 30 });
          this.$store
            .dispatch(
            ...
            .then(() => {
              ...
              getCurrentUser().then((res) => {
                if (res.code == 0) {
                  Cookies.set("deptLevel", res.data.deptLevel);
                }
              });
              this.$router.push("/cashome");
            })
            .catch(() => {
              this.loading = false;
              this.getCode();
