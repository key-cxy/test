            ...
            <div class="search" style="flex-direction: column">
              <div class="searchRow">
                ...
                <div class="searchCol">
                  <span>所属大类：</span>
                  <div>
                    <el-select :disabled="!classDisable"  style="width: 100%" v-model="bigClass" @change="handleBigClass">
                      <el-option
                        v-for="(i,index) in bigClassList"
                        :key="index"
                        :value="i.id"
                        :label="i.name"
                      ></el-option>
                ...
                <div class="searchCol">
                  <span>所属小类：</span>
                  <div>
                    <el-select :disabled="!classDisable"  style="width: 100%" v-model="smallClass" @change="handleSmallClass">
                      <el-option
                        v-for="(i,index) in smallClassList"
                        :key="index"
                        :value="i.id"
                        :label="i.name"
                      ></el-option>
              ...
              <div class="searchRow">
                <div class="searchCol">
                  <span>立案名称：</span>
                  <div>
                    <el-select :disabled="!classDisable"  style="width: 100%" v-model="filingName" @change="getFilingCondition">
                      <el-option
                        v-for="(i,index) in filingNameList"
                        :key="index"
                        :value="i.id"
                        :label="i.name"
                      ></el-option>
              ...
              <div class="searchRow">
                ...
                <div class="searchCol">
                  ...
                  <div>
                    ...
                    >
                      <el-option
                        v-for="(item,index) in disposerList"
                        :key="index"
                        :value="item.id"
                        :label="item.nickname"
                        >
...
<!--          </div>-->
                ...
                <div class="searchCol">
                  ...
                  >
                    <template slot="url" slot-scope="text">
                      <img @click="changeImg(text)" :src="text" style="width: 30px; height: 30px" />
                    </template>
    ...
    <el-button type="primary" @click="handleYichange">确 定</el-button>
  </span>
      </el-dialog>
      <el-dialog
        title="延期"
...
export default {
  ...
  methods: {
      ...
      await handleCaseView({ caseNo }).then((res) => {
        if (res.code == 200) {
          ...
          if (this.phStatus == 1) {
            ...
            if(res.data.buttonList && res.data.buttonList.length){
              if(res.data.buttonList[0].deptEditPermission == 1){
                this.assDisable = true
                this.classDisable = true
                this.deptDisable = true
              }else if(parseInt((res.data.buttonList[0].deptEditPermission % 10000) / 1000)==2){
                if(parseInt((res.data.buttonList[0].deptEditPermission % 1000) / 100)==1){
                  this.assDisable = true
                } else {
                  this.assDisable = false
                }
                if(parseInt((res.data.buttonList[0].deptEditPermission % 100) / 10)==1){
                  this.classDisable = true
                } else {
                  this.classDisable = false
                }
                if(parseInt(res.data.buttonList[0].deptEditPermission % 10)!=0){
                  this.deptDisable = true
                } else {
            ...
            this.assessmentUnitTemp = getPathById(this.assessmentUnitList,this.assessmentUnit)
            if(this.filingName){
    ...
    handleSave(button) {
      ...
      // if (deptLevel == 3) {
      //   if (!this.areaVal[1]) {
      //     this.checkInfo("请选择所属区域的下属街道");
      //     return;
      //   }
      // }
      // if (deptLevel == 4) {
      //   if (!this.areaVal[2]) {
      //     this.checkInfo("请选择所属区域的下属社区");
      //     return;
      //   }
      // }
      // 暂时放开
      if (button.operateType == 'pizhuanchuzhi'&& this.disposer.length == 0) {
        this.checkInfo("处置人员必选");
        return;
      }
      // if (this.areaVal.length == this.oldAreaVal.length) {
      //   this.checkInfo("没有派发到下级区域时，处置人员必选");
      //   return;
      // }
